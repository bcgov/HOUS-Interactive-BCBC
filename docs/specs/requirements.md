...
4. WHEN displaying search results, THE Application SHALL sort results primarily by content hierarchy level (Part > Section > Subsection > Article > Table/Figure > Note > Glossary), then by relevance score within the same level, allowing a lower-level item to override hierarchy only when its relevance score exceeds the higher-level item by more than 3x
