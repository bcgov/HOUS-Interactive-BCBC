...
- **Cross-division suffix ("of Division X"):** For `long`-format references in *prose contexts* (articles, appendix, application notes), `formatInternalReference()` automatically appends "of Division X" when the target division differs from the current article's division (e.g. a Division A article referencing "Section 2.3 of Division C"). This uses the `currentDivision` extracted from `ReferenceRenderContext`. **Table cells use `kind: 'table'` context, which suppresses auto-generation of the suffix** — table cells rely on inline source text (e.g. ` . of Division B` written after the marker) when the suffix is needed. To avoid duplication, `getCrossReferenceDisplayText()` detects when a generated "of Division X" suffix matches inline text immediately following the marker and consumes that inline text. Application note references (`part_appendix` kind) also show "of Division X" in `long` format when cross-division, with `divBV2` displayed as "Division B" (canonical first letter of division code).
...
| Cross-division "of Division X" missing in article text | `formatInternalReference()` only adds suffix when `currentDivision` is known and differs from target. Check that `renderContext` (with `kind: 'article'`) is passed through `ArticleBlock` → `ContentRenderer` → `SentenceBlock` → `parseTextWithMarkers`. The `currentDivision` is extracted from `renderContext.referenceId` inside `getCrossReferenceDisplayText()`. |
| Duplicate "of Division X" in article text | Source text has an inline `. of Division X` after the marker AND the renderer generated the same suffix. `getCrossReferenceDisplayText()` consumes the inline text via the `divisionSuffix` check. If the duplicate persists, verify the regex `/^\s*\.?\s*of Division ${divLabel}\b/i` matches the exact whitespace/punctuation pattern in the source. |
| "of Division X" appearing in table cells where it shouldn't | `TableBlock` converts its incoming `renderContext` to `{ kind: 'table', referenceId }` via `toTableRenderContext()`. Both `getCrossReferenceDisplayText()` and the `CrossReferenceLink` `currentDivision` prop skip division suffix generation for `kind: 'table'`. Check that all `renderRows`, `renderBodyRowsWithColTracking`, and direct `renderCellContent` calls in `TableBlock` use `tableRenderContext`, not the raw `renderContext` prop. |
| Appnote shows full title instead of "of Division B" in long format | `CrossReferenceLink` resolves appnote display text asynchronously. For `long` format, it shows "of Division X" only when `currentDivision` prop is set and differs from the note's canonical division letter (first char of division code). Check that `parseTextWithMarkers` passes `currentDivision` to `CrossReferenceLink` and that the article's `renderContext.kind !== 'table'`. |
...
| `[LIST:variable]` or `[LIST:symbol]` terms appear **bold** | `.structuredList__term` has global bold weight; variable/symbol types override this to `regular` weight in `StructuredListBlock.css`. Check that the override selector `.structuredList--variable .structuredList__term` is present. |
| `[LIST:variable]` nested inside a bullet `<li>` is over-indented | `li .structuredList--variable` in `StructuredListBlock.css` controls indentation — should be `padding-left: 3rem` (≈ 13 spaces, matching bullet text alignment). |
